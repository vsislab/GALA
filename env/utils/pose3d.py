...
def QuaternionRotatePoint(quat, point):
...
def QuaternionRotatePointInverse(quat, point):
  """Performs a inverse rotation by quaternion.

  Inverse rotate the point by the quaternion using quaternion multiplication,
  (q^-1 * p * q), without constructing the rotation matrix.
  ...
  quat_inverse = transformations.quaternion_inverse(quat)
  q_point_rotated = transformations.quaternion_multiply(
      transformations.quaternion_multiply(quat_inverse, q_point), quat)
  return q_point_rotated[:3]
